...
impl BookRepository for BookRepositoryImpl {
    ...
    async fn find_by_filters(&self, params: BookFilterParams) -> Result<Vec<Book>> {
        ...
        match (params.status, params.author_pattern.as_ref(), params.title_pattern.as_ref()) {
            (Some(status), Some(author), Some(title)) => {
                sqlx::query_as!(
                    Book,
                    r#"
                    SELECT 
...
mod tests {
    ...
    async fn test_search_books(pool: PgPool) {
        ...
        assert!(results.iter().any(|book| book.title.contains("Unique Search")));
    }
    ...
    async fn test_update_status(pool: PgPool) {
        ...
        let success = repo.update_status(99999, BookStatus::Available).await.unwrap();
        assert!(!success);
    }
}