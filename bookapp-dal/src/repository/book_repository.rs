...
impl BookRepositoryImpl {
    ...
    pub async fn find_all(&self, limit: i64, offset: i64) -> Result<Vec<Book>> {
        self.find_all_with(&*self.read_pool, limit, offset).await
    }

    pub async fn find_all_with<'e, E>(&self, exec: E, limit: i64, offset: i64) -> Result<Vec<Book>>
    where
        E: Executor<'e, Database = Postgres>,
    {
        debug!(
            "Getting books from database with pagination: limit={}, offset={}",
            limit, offset
        );
    ...
    pub async fn list_ids(&self, limit: i64, offset: i64) -> Result<Vec<Uuid>> {
        self.list_ids_with(&*self.read_pool, limit, offset).await
    }

    pub async fn list_ids_with<'e, E>(&self, exec: E, limit: i64, offset: i64) -> Result<Vec<Uuid>>
    where
        E: Executor<'e, Database = Postgres>,
    {
...
mod tests {
    ...
    async fn test_find_all_books(pool: PgPool) {
        ...
        let books = repo.find_all(10, 0).await.unwrap();
    ...
    async fn test_list_book_ids(pool: PgPool) {
        ...
        let ids = repo.list_ids(5, 0).await.unwrap();
